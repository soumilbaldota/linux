...
// static int superfork_copy_thread(struct task_struct *p,
//                                  struct task_struct *src_task,
//                                  u64 clone_flags)
// {
...
//     /* Copy frozen user-mode register state from src_task */
//     *childregs = *srcregs;
//     childregs->regs[0] = 0;                          /* return 0 in child */

//     /* Copy TLS from src_task's live register, not stale thread struct */
//     *task_user_tls(p) = src_task->thread.uw.tp_value;

//     if (clone_flags & CLONE_SETTLS)
//         p->thread.uw.tp_value = src_task->thread.uw.tp_value;

//     /*
//      * x19/x20 = 0: this is a user task, not a kthread.
//      * ret_from_fork checks x19; if NULL it goes straight to ERET.
//      */
...
//     /* ── 1. Locate the fork_frame embedded below pt_regs ── */
//     childregs = task_pt_regs(p);
//     fork_frame = container_of(childregs, struct fork_frame, regs);
//     frame      = &fork_frame->frame;

//     /* ── 2. Set up the kernel-stack frame __switch_to_asm will pop ── */
//     frame->bp       = encode_frame_pointer(childregs);
//     frame->ret_addr = (unsigned long)ret_from_fork_asm; /* entry_64.S stub */
//     frame->bx       = 0;   /* NULL fn → ret_from_fork goes straight to IRET */

//     p->thread.sp        = (unsigned long)fork_frame;
//     p->thread.io_bitmap = NULL;
//     clear_tsk_thread_flag(p, TIF_IO_BITMAP);

//     /* ── 3. Segment registers from src_task, not current ── */
//     /*
//      * current_save_fsgs() flushes live CPU state into current->thread.
//      * Since we're copying from src_task (which is frozen / ptrace-stopped)
//      * its thread struct already holds the correct saved values.
//      */
...
//     /* ── 4. Shadow stack (CET). 0 stack_size → no new shstk ── */
//     unsigned long new_ssp = shstk_alloc_thread_stack(p, clone_flags, 0);
//     if (IS_ERR_VALUE(new_ssp))
//         return PTR_ERR((void *)new_ssp);

//     /* ── 5. Clone FPU/XSAVE state from src_task ── */
//     /*
//      * fpu_clone() normally copies from current. Because we need to copy
//      * from src_task, manually copy the FPU state then inform fpu_clone
//      * there is no kernel fn (NULL) so it follows the user-task path.
//      */
//     fpu_clone(p, clone_flags, NULL, new_ssp);

//     /* ── 6. PKRU: read from src_task's saved value (it is scheduled out) ── */
//     p->thread.pkru = src_task->thread.pkru;

//     /* ── 7. Copy full user pt_regs from frozen src_task ── */
//     *childregs    = *srcregs;
//     childregs->ax = 0;      /* child returns 0 from the syscall */

//     /* ── 8. Ptrace breakpoints start clean ── */
//     memset(p->thread.ptrace_bps, 0, sizeof(p->thread.ptrace_bps));

//     /* ── 9. LAM: inherit lock if sharing the same MM without vfork ── */
...
// #else
// # error "superfork: unsupported architecture"
// #endif
